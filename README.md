...
This code example demonstrates the use of the Flash PDL driver to perform basic read, write, and erase operations on code flash, and update sector data online. The steps are logged on a UART terminal emulator.

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce240753-code-flash-sector-data-updating)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce240753-code-flash-sector-data-updating](https://github.com/Infineon/mtb-example-CE240753-code-flash-sector-data-updating)" application with the desired name "Code_flash_sector_update" configured for the *KIT_T2G_C-2D-6M_LITE* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_T2G_C-2D-6M_LITE --app-id mtb-example-CE240753-code-flash-sector-data-updating --user-app-name code_flash_sector_update --target-dir "C:/mtb_projects"
   ```
...
4. After programming, the application starts automatically. Confirm that "PDL: Code Flash Sector Data Updating" is displayed on the UART terminal.
...
In this example, the Flash_PDL driver is configured to read the entire sector data from target sector (sector_0), program that data into an empty sector (sector_1), erase the target sector, then read back the source data from sector_1. The data is processed (some values are modified), and then programmed back into the target sector (sector_0). 
...
Resources  | Links
-----------|----------------------------------
Application notes  | [AN235305](https://www.infineon.com/assets/row/public/documents/10/42/infineon-an235305-getting-started-with-traveo-t2g-family-mcus-in-modustoolbox-applicationnotes-en.pdf) – Getting started with TRAVEO&trade; T2G family MCUs in ModusToolbox&trade;　<br /> [AN241720](https://www.infineon.com/document-promo/infineon-an241720-getting-started-with-xmc5000-mcu-on-modustoolbox-software_1071f992-eb73-4dce-94ad-e84c41407bfc) – Getting started with XMC5000 MCU on ModusToolbox&trade; software
...
Document title: *CE240753 – PDL: Code flash sector data updating* 
